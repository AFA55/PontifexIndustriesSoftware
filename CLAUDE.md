...
- **Authenticated client → API calls must send `Authorization: Bearer <access_token>`** (from `supabase.auth.getSession()`); `requireAuth()` reads the bearer token, NOT cookies. See `app/dashboard/my-profile/page.tsx` for the pattern.
- **"Save password" on login (don't revert):** the fetch/SPA login must do a FULL navigation on success — `window.location.assign(target)`, NOT `router.push` — or Safari/iOS/Firefox never fire the native "Save password?" prompt (no navigation = no save signal). Chromium also gets an explicit `navigator.credentials.store(new PasswordCredential(...))` push. Inputs keep `autoComplete="username"`/`"current-password"`; email pre-fills from `pontifex.lastEmail`. (deep-research-verified Jun 14, 2026; iOS Capacitor WKWebView is the one place neither works — it uses native Face ID.)
- **Email branding reads `tenant_branding`, NOT `tenants`:** the logo + `secondary_color` live in the `tenant_branding` table (same source the login page uses); the `tenants` table only has a sparse `primary_color`/`logo_url` (often null). `getTenantEmailBranding()` in `lib/email.ts` reads `tenant_branding` first (Patriot: logo + red `#DC2626` + navy `#1E3A5F`), falls back to `tenants`, then platform purple. All transactional emails share `emailHeader()` (white band + logo + red→navy accent bar).
- **Web biometric login: REMOVED (Jun 14, 2026).** We built a custom WebAuthn/passkey "Use Touch ID" flow on the website then removed it at the founder's request — the browser's own password manager (+ native passkey autofill) already handles "remember username/password/passkey," so the custom layer was redundant. Deleted: `PasskeySettings`, `PasskeyLoginButton`, `BiometricEnrollPrompt`, `lib/webauthn-client.ts`, `lib/webauthn.ts`, `app/api/auth/webauthn/*`. The login form just pre-fills the last username (`pontifex.lastEmail`) and uses `autoComplete="username"`/`"current-password"` so the browser saves/fills. (Dormant leftovers: the `webauthn_credentials` table + the `auth_user_id_by_email()` function — the function is STILL USED by `setup-account/complete`, do not drop it. `@simplewebauthn` deps are unused, removable later.) **The iOS APP keeps its separate NATIVE Face ID** (`lib/biometric.ts`, gated by `isNativeApp()`) — that was NOT removed.
