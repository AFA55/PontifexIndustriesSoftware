...
- [~] **Cut iOS Build 9** — ✅ BUILT: bumped 1.0.3/8 → **1.0.4/9** (`4c95d061`), `cap sync ios`, **archive + export DONE by Claude** (manual signing, profile "Pontifex App Store Distribution" — ARCHIVE/EXPORT SUCCEEDED). **Signed IPA ready: `~/Desktop/Pontifex-Build9-v1.0.4.ipa`** (also `/tmp/PontifexExport/App.ipa`), verified v1.0.4/build 9. **REMAINING (founder, ~2 min — Claude's computer-control dialog timed out twice):** (1) open Transporter → drag the Desktop IPA → Deliver; (2) ASC → version 1.0.4 → attach build 9 → What's New → Submit for Review. ⚠️ Check the Apple Developer License Agreement re-accept in ASC if the upload won't process. (Web features already live via Vercel — Build 9 = native splash + Face ID plugin only.)
