...
### 🔴 P0
- [x] ~~**Work items are written TWICE on a multi-day job**~~ FIXED Aug 14 — it was worse than doubling. `GET /work-items` had **no filter at all** and returned every day and every operator; day-complete loaded the lot into "what I did today" and submitted it, so day 2 resubmitted day 1, day 3 resubmitted 1+2. Pratt: 200 LF → 1,000 → 2,800. Work items now carry `work_date`, both write paths replace on `(job, operator, work_date)`, and the read is scoped by `?date=&mine=1`.
- [ ] **15 duplicated work-item rows would make the PRINTED total read high.** *(Framing corrected by the founder, Aug 15 — the system does NOT decide billing. Project managers and admin write the invoice by hand from the printed work order + work performed tickets. Nothing is auto-billed, and invoicing is explicitly not a priority. What matters is that **work performed accumulates accurately and reads cleanly**, because a person is copying it onto an invoice.)* A later day repeats an earlier day's exact figure on the same job. No clean mechanical discriminator — 9 of the 18 carry full `details_json`, indistinguishable from operator-entered work — so this needs someone who knows what was actually cut. Parkk 424813 (WALL SAW 132 first logged 8/4, repeats 8/5, 8/6, 8/13; 31 from 8/7 → 8/13; 57 from 8/10 → 8/13; 44 from 8/12 → 8/13; CORE DRILL 7 from 8/4 → 8/13) · Pratt 895358 (SLAB SAW 200 from 8/11 → 8/13; 300 and 700 from 8/12 → 8/13) · Parkk 402357 (SLAB SAW 190 from 8/5 → 8/6, 8/7) · Collins 124747 (SLAB SAW 106 from 8/5 → 8/6) · Door Systems 364026 (HAND SAW 2, PUSH SAW 6, both 8/5 → 8/6) · Harper 631148 (CORE DRILL 7, ELECTRIC CORE DRILL 1, both 8/4 → 8/5) · J. Davis 859542 (SLAB SAW 1 from 8/7 → 8/10). Backup: `_work_items_backup_20260814`.

### 🔴 AUG 15 — MODEL CORRECTION: the schedule board is a DISPATCHING tool

- [ ] **🔴 A duplicated card is another CREW on one job, not another job.** Founder: *"There aren't 4 Parkk jobs for Logistics Drive… same job, just operators available that can work at that job as well. The schedule board is a dispatching tool — we need to get that clear."* He duplicates a ticket to staff a second crew, or to send an operator who finished early to help. Today each duplicate becomes a separate `job_orders` row. **Measured on the live Parkk job at 520 Logistics Dr: ONE job, FOUR rows, 19 work items and 17 daily logs split across them** — so billing sees a quarter of the footage, four sheets print for one job, and Active Jobs shows four cards. Worse, the rows form a CHAIN (each duplicate parented to the previous duplicate, not the root), so anything that rolls up by root already misses rows — check whether `original.parent_job_id ?? original.id` in the duplicate route is wrong or whether this data predates it. Plan, options and a migration outline: `docs/plans/DISPATCH_MODEL_AND_WORK_PERFORMED_TICKETS.md`. Recommendation is Option A — duplicating writes a `job_daily_assignments` crew row, not a job. Needs founder answers on: does a second crew get its own printed sheet, can it have a different arrival time, and does Active Jobs show one card or one per crew.
- [ ] **Work Performed Tickets page** — pick a date range, see every ticket grouped by person, tick and print. Replaces hunting Active Jobs then Completed Jobs job by job. Wireframes for both the screen and the printed sheet in the same doc. Two rules INVERT vs the job ticket and will be introduced silently by copy-pasting `buildTicketDays`: shop time must be **included** here (it is paid time on a payroll sheet), and `quantitiesFrom: 'lead'` must **not** apply (it would print every helper an empty page). Founder deferred the print reminder: *"eventually set reminders and ask admin if they want to print, but that's later on."*

### 🆕 AUG 15 PM — PM-view testing (built, not pushed)

- [x] **"Unauthorized. Invalid or expired session." when printing a work ticket — the message was wrong.** Founder, Aug 15: *"I just tried to print work ticket for this job and it said error… this is what PMs and admin were telling me when they tried to print something."* The Supabase auth log for that exact second (21:49:58, matching the Vercel 401 on `/api/admin/jobs/cc855000…/work-ticket`) says `GET /user 403 — "token is malformed: token contains an invalid number of segments"`. **Not expired — malformed.** The bearer sent was not a JWT at all, and `resolveAuth` flattened every auth failure into the one "invalid or expired" sentence, so the office reported a session problem and everyone looked in the wrong place. Shape of the fault in 24h of Vercel logs: 401s arrive in CLUMPS — ten schedule-board routes together, twice; the work ticket once — i.e. one bad token at page mount, not per-route permission disagreement. **Root cause of the malformed string is still UNKNOWN.** Shipped: `lib/authed-fetch.ts` (validate the token is three dot-separated segments before sending; on 401 force one `refreshSession()` and retry once; typed `SessionExpiredError` so a dead session gets a login button instead of a red sentence), adopted by the work-ticket page, the job-order print page and `JobDocuments` (all three call sites). Plus a server-side diagnostic in `lib/api-auth.ts` logging only the token's SHAPE — length, segment count, `had_bearer_prefix`, never any token bytes — so the next occurrence names its own cause. One live candidate was found and closed on the way: the parse was `authHeader.replace('Bearer ', '')`, an unanchored exact-literal match, so `bearer <jwt>` or a double space left the word inside the token — exactly the "malformed" error observed. Now `/^\s*bearer\s+/i` + `.trim()`.
- [x] **"The PM should have known they haven't completed their job."** Founder, Aug 15: *"that job is done but operator hasn't pressed submit or completed job — we should have a button that sends notification to them to complete their job."* The job view already SAID it ("Work logged; day not wrapped up yet") and offered no verb. New: `lib/closeout-nudge.ts` + `POST /api/admin/jobs/[id]/closeout-nudge` + `components/JobCloseoutNudge.tsx` — a banner above the day list naming the open days, with **Remind crew to close out**, sibling to the waiver nudge (same dispatcher, same one-press-per-job-per-hour dedup, same audit row). Goes to the operator who **worked the open day**, not whoever holds the job today. Two guardian BLOCKERS were caught and fixed before it went anywhere: (1) `daily_job_logs` holds one row **per operator** per day, so a lead pressing Done leaves the helper's row null forever — reading one row called finished days open and would have texted Devin Scroggs to close out Aug 11–13 on Pratt, two of which Conrade had already closed; completion is now aggregated across every row for the day. (2) The client passes work items with no date (grouped by day NUMBER), so one day bucketed twice and the banner read "2 days never wrapped up · Aug 12 and Day 3"; loop 1 now adopts the matched log's date, and the client and server paths were re-verified to return identical results on the real Pratt rows.
- [ ] **⚠️ Your `andres@pontifexindustries.com` super_admin cannot open Patriot jobs.** That profile is `super_admin` on tenant `b27d9ca5` (PONTIFEX); every live job is on `ee3d8081` (PATRIOT). The work-ticket route refuses a tenant mismatch with a 404 "Job not found" — so once the token fix lands, printing a Patriot job from that account will fail with a *different* message. In the Aug 15 screenshot the founder was signed into the Pontifex portal while printing a Patriot job. **Not changed unilaterally** — letting one tenant's account read another's is a security decision for the founder. Options: use the Patriot login (`andres@patriotconcretecutting.com`, operations_manager) for Patriot work, or decide that a super_admin with a tenant may read across tenants. Note `super@pontifex.com` is a super_admin pinned to PATRIOT and does work.
...
- [ ] **Timecard correction-request UI doesn't match the platform.** Fields cramped, spacing tight, styling off from everything around it. Rebuild with the standard field/spacing patterns (see `docs/reference/UI_CATALOG.md`), bigger touch targets — this is filled in on a phone.
- [ ] **Tools menu: drop the JSA form, promote the concrete weight calculator.** JSA isn't used. Put the calculator directly under "View Time Cards" where the crew will find it — it genuinely helps them. Its UI is too plain for the rest of the platform: bigger fields, clearer labels, make the result the hero.
- [ ] **Rate My Crew — test data staged.** `TEST-2026-000105` puts the founder in as lead with Demo Helper so the rating flow can be walked. (Reminder of the decision already taken: helper reviews are aggregate-only and suppressed below n=3; detailed reviews are supervisor-visible.)

### 🟠 P1 — from today's batch, not yet built
- [ ] **"Work Performed Tickets" — the sheet that replaces the staple.** *(founder, Aug 15 — plan: `docs/plans/OFFICE_PAPERWORK_AND_SITE_CONDITIONS.md` §Feature 1)* Today's ticket is JOB-centred (one job, its days, its crew). Payroll is PERSON-centred, so the office staples two documents together by hand. New tab + `GET /api/admin/work-performed-tickets` (`requireAdmin`, page guard the SAME three roles — do not use `isAdmin()`, that is weak join #1): every crew member, the jobs they were on, hours, for a day / week / custom range, multi-select, one Print → one portrait sheet per person. Reuses `lib/work-ticket.ts` helpers + `lib/work-items-format.ts` + the job ticket's print CSS; NEW is `lib/person-ticket.ts` (the transpose) and multi-sheet `break-after: page`. **Two rules invert and both will be got wrong by copy-paste:** shop cards are EXCLUDED from the job ticket (`isShopCard`, never job labor) and must be INCLUDED here (they are paid hours); and `quantitiesFrom: 'lead'` must NOT apply — each person prints their own entries, subtotal labelled "not the job total", with "lead that day" named. Quick Notes / day notes / helper notes ARE allowed (internal office document, never crosses `toCompletionPdfWorkItems` or `stripInternalNotes`); wages are not. Payroll week is Sat→Fri and nothing reads `week_start_day` — make custom range the primary control.
- [ ] **Job site conditions, captured on the FIRST day on site.** *(founder, Aug 15 — plan: `docs/plans/OFFICE_PAPERWORK_AND_SITE_CONDITIONS.md` §Feature 2)* "We didn't know they had to walk five hundred feet from the truck, drop a generator, and each hole was on a different level." A prominent free-text box — *"write down anything that prevents you or slows you down from doing your job"* — plus six one-tap answers (distance from truck, water site/truck + hose ft, power site/generator, levels, does the scope match, photos). The writing is the point; the buttons exist so the box doesn't carry the easy parts. **Wire dictation (`VoiceMemoNotes`) on day one** — a crew in gloves will not type a paragraph. NEW table `job_site_condition_reports` (tenant_id + RLS via the SECURITY DEFINER helpers, never `user_metadata`), NOT an extension of `job_orders.jobsite_conditions`: that column is 100% office-authored (verified — all 29 non-empty prod rows carry the same 19 schedule-form step-8 keys), it is one object per job, and it is copied by duplicate + resubmit. Keeping the plan and the observation apart is what makes "does the scope match?" answerable. "First day" = `dayPosition === 1` from `job_workday_evidence` (the same expression `set_daily_log_day_number()` uses) AND no report exists on the job — computed server-side, per job not per person, soft card on days 2–3, gone after. **Must never block `handleSubmit`** — same reasoning as the Aug 3 photos fix; a required field at 5pm gets garbage typed into it.
...
- [x] ~~**David sees ZERO rows in twelve tables**~~ FIXED Aug 14 — 13 tenant-scoped supervisor SELECT policies.
- [ ] **TRIPWIRE — the day job costing goes live, supervisor (and salesman, shop_manager, inventory_manager) must stop reading `job_orders` directly.** They can all pull `total_cost`, `gross_profit`, `total_revenue`; `timecards` exposes `labor_cost`, GPS, clock-out photos, lateness flags and `admin_notes`. Every one of those money columns is zero today, which is the only reason this is acceptable. Column-level `REVOKE` **cannot** fix it — every logged-in user shares the single `authenticated` Postgres role, so revoking a column takes it from admins too. The end state is a view with a safe column subset, or serving those pages through the server API with `supabaseAdmin` like the rest of the codebase.
- [ ] **Pre-existing and bigger than the above: `profiles_select_all_authenticated` lets EVERY authenticated user read every profile in the tenant** — including `hourly_rate`, `commission_rate` and `date_of_birth` (populated on 8 people). Now that pay is editable in Team Profiles, this is where a wage would actually leak from. Fix before rates are entered at scale.
- [ ] **Eight ad-hoc `_*_backup_*` tables are accumulating in `public`** from manual prod surgery. RLS on with zero policies, so fail-closed and not a leak — but they should be swept once the changes they insure are confirmed good.
...
### 🆕 AUG 16 — founder batch (isolation, backups, stability)

- [x] **A Supabase blip told the whole company they were logged out.** Caught LIVE 16:50–16:52 UTC by the token-shape diagnostic: 196 auth failures vs 62 successes, every token perfect (765 chars, 3 segments, Bearer prefix present), `reason` a bare fetch error carrying only the Supabase Auth URL. `resolveAuth` calls Supabase Auth over the network on EVERY request and treated a failure of the CALL as a failure of the TOKEN → "your session has expired, please sign in again", advice that cannot work because signing in hits the same service. Now returns **503 `auth_service_unavailable`** with "your login is fine — this is on our side"; genuine bad tokens still 401. Also removed the amplification: `authedFetch` forced a refresh on 401, so every client was making 2–3 calls to the failing service. Predicted as R9 by the stability audit hours before it fired.
- [x] **The overdue-ticket gate is gone** (founder, Aug 16): *"if they forget to complete tickets allow them to continue on their current ticket."* An operator with an unfinished ticket from a previous day was 409'd out of starting today's job. It punished the person on the jobsite for paperwork the OFFICE needs, and had already stranded Nate for a week over an `on_hold` job he was not allowed to work — the growing exception list was the tell. The modal is now a reminder ("You have a ticket to finish… go ahead and start today's job"), and late entry is correct by design: work items carry their own `work_date`, so a day filed later books to the day it was worked.
- [x] **`super@pontifex.com` removed from the login bundle.** It was an ACTIVE `super_admin` **inside the PATRIOT tenant** with its password published in a Client Component. **⚠️ THE AUTH USER ITSELF IS NOT YET DELETED** — Supabase was unreachable when this shipped. Delete it the moment the DB responds.
- [ ] **⚠️ `admin@pontifex.com` / `PontifexDemo2026!` is STILL published** in `app/login/page.tsx` and is a real admin — it can read every job, customer and timecard in its tenant. Rotate it, or move all demo accounts to a demo-only tenant.
- [x] **`tenant_id` is now NOT NULL** on timecards, job_orders, work_items, daily_job_logs, invoices, customers, job_daily_assignments, job_crew. Zero rows needed fixing — every live write path already set it. Converts silent, permanent invisibility into a loud error. `profiles` deliberately excluded (super_admin may be tenant-less; that is a product decision).
- [x] **`REVOKE SELECT ON equipment_units FROM anon`** — the key in the public JS bundle was reading Patriot's asset register unauthenticated: 4 rows, all with `manufacturer_serial`, plus `purchase_price`. Verified closed.
- [x] **Deleted `app/api/work-items/route.ts`** — decoded a bearer and then did NO role, tenant or assignment check before `.update().eq('id', job_order_id)` on job_orders. Zero callers.
- [x] **Deleted `EMERGENCY_FIX_RLS.sql`, `COMPLETE_FIX_RLS.sql`, `COMPLETE_OPERATOR_ACCESS_FIX.sql`** — the first DISABLEs RLS on `profiles`, and since `is_admin()` reads `profiles.role`, any operator could then make themselves super_admin.
- [x] **"Work performed saved!" when nothing saved.** The `!res.ok` branch logged to console and fell through to the success toast and a navigation. Nothing ever re-sent the localStorage copy. An operator in a parking garage lost a day of cutting and was told it saved. Now stops on the page with "Not saved yet" + Retry.
- [x] **Nightly off-site backup built** (`/api/cron/backup-export`, 08:00 UTC) — every table discovered at runtime, NDJSON + gzip, plus Storage objects, PUT to an S3-compatible bucket outside Supabase via hand-rolled SigV4 (verified against AWS's published test vector). Fails LOUDLY when unconfigured. **Founder is setting up Cloudflare R2; needs `BACKUP_S3_*` in Vercel.**
- [x] **Fixed `jest.setup.js`** — unguarded `window` + no TextEncoder/WebCrypto. `lib/email.test.ts` and `lib/email-links.test.ts` had been failing to LOAD for weeks. 53 suites / 1,043 tests now pass.

#### Open — founder decisions and next work
- [ ] **Hub access for the platform owner** (founder, Aug 16): he works for Patriot and logs in with that email; he also owns Pontifex and needs a "hub" to manage client companies. Decision: **one email = one company (Option A)**, plus a SEPARATE way to reach the hub that is not a company code — *"Pontifex is owner of the software and I need a way to access a hub."* Research how multi-brand parent orgs actually structure this before building.
- [ ] **Telegram alerting + AI triage loop** (founder, Aug 16): Sentry → Telegram so he is notified directly when something breaks; an agent from the Pontifex hub analyses the error, proposes a fix, he approves or rejects in chat. Also a user-facing "submit a bug / request a change" surface that feeds the same channel. Motivation: *"I don't access the hub every day and I also don't get notifications."*
- [ ] **Offline capture for the field** (founder, Aug 16): *"they try to complete information but they don't have service — how can we resolve that?"* Today the localStorage copy is only ever read to PREFILL, and `NetworkMonitor`'s "changes will sync when you reconnect" does nothing. Needs a real outbox: queue the submission, retry on reconnect, and show honest pending state. This is the other half of the "Work performed saved!" fix.
- [ ] **Make the "our team has been notified" message true** — 0 of 60 error boundaries call `Sentry.captureException`. ~1h, closes a 2-month blind spot.
- [ ] `operator_status_history` — 45 failures since Aug 4; three code/schema mismatches (NOT NULL `job_order_id`, a CHECK that rejects 4 of 5 accepted statuses, `.insert()` where the UNIQUE demands upsert).
- [ ] `helper_work_logs` — 64 failures since Jun 23; the PostgREST embed traverses an FK into `auth.users` for a `profiles` column and can never resolve.
- [ ] Operator home screen shows **0 jobs after ~8pm** — `toISOString().split('T')[0]` in `app/dashboard/page.tsx:301`. `lib/dates.ts` already has `toLocalYMD`.
- [ ] `UNIQUE (job_order_id, operator_id, work_date, work_type)` on `work_items` — the DB backstop the quantity-tripling P0 never had. 15 duplicate rows still live.

#### Aug 16 PM — built while the founder was away
- [x] **A database hiccup was silently removing the company filter.** `lib/get-tenant-id.ts` ended `catch { return null }`, and all 101 callers read that as `if (tenantId) query.eq('tenant_id', tenantId)` — i.e. **no filter at all**. Any transient failure reaching Supabase downgraded a company-scoped query into a platform-wide one, for every role, on updates and deletes as well as reads, silently. **Live condition, not theoretical:** Supabase was unreachable for over an hour this afternoon. Now throws; 126 of 136 call sites already return a clean 500, the other 10 surface Next's own 500. The legitimate null (query succeeded, user genuinely has no tenant) is preserved. Found by the parent-org research agent, which measured the true blast radius at **263 sites**, not the ~18 first reported.
- [x] **The health check went down with the thing it was checking.** Three dependency checks in series, no timeouts, ~25s each → 46 of 46 requests returned 504 in fifteen minutes and it never once reported "degraded". Now deadlined at 3s, parallel, always answers; 503 only for genuinely down so latency does not page anyone.
- [x] **Offline outbox** — work submitted with no signal queues on the phone and sends itself on reconnect, including after the app is closed. Safe because the server replaces on (job, operator, work_date). Never silently drops: after 7 days/25 attempts it is surfaced for a human.
- [x] **"Our team has been notified" is now true** — `lib/report-error.ts` wired into the three shared error boundaries → console + Sentry + `/api/log-error` → Telegram, with a 15-minute per-fingerprint flood guard. Also fixed the three operator-reachable boundaries sending crashed users to `/dashboard/admin`, a page operators cannot open.
- [x] **Telegram alerting built** (`lib/telegram.ts`) — dormant until `TELEGRAM_BOT_TOKEN` + `TELEGRAM_ALERT_CHAT_ID` are set. Setup: `docs/playbooks/SETUP_BACKUPS_AND_TELEGRAM.md`.
- [ ] **⚠️ `super@pontifex.com` still not deleted** — password is out of the bundle, the auth user remains. Supabase has been unreachable since ~16:20 UTC. **Do this first when the connection returns.**

#### The hub decision — research complete, see `docs/plans/PARENT_ORG_AND_HUB_ACCESS.md`
Recommendation: **separate Pontifex identity + a hub that MANAGES companies without reading them + an explicit, time-boxed, logged "Open this customer" action.** The founder keeps his Patriot employee account for Patriot work — that is the correct model, not a workaround, and it keeps the access log meaningful. Research basis: where the vendor does not own the customer org, every serious platform gates access on consent and expiry (Shopify collaborator codes, Microsoft GDAP — which explicitly dismantled standing partner access — AWS AssumeRole, Salesforce grant-with-expiry + audit trail). "Manage" and "read" are separable: create, brand, bill, monitor and back up a tenant without opening a job.
