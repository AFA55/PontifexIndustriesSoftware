...
- [~] **Cut iOS Build 9** — ✅ BUILT + ✅ UPLOADED: 1.0.3/8 → **1.0.4/9** (`4c95d061`), cap sync, archive+export (manual signing), **DELIVERED to App Store Connect via Transporter Jun 21 3:32 PM** ("THE APP IS PROCESSING"). **REMAINING (after Apple finishes processing ~5–30 min):** ASC → create version 1.0.4 → attach build 9 → What's New → export-compliance → **Submit for Review**. (Web features already live via Vercel — Build 9 = native splash + Face ID plugin only.)
