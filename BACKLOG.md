...
- [ ] **🎨 BIG (IN PROGRESS): full color-palette → ENTIRE-app theming.** Founder: changing colors in Settings must recolor the WHOLE app, "not just some areas," for every company code incl. Pontifex. Foundation done (brand tokens + admins can edit). Brand-sweep campaign by impact (visible screens first; debug/rare-modal long-tail last):
  ...
  - [ ] **Remaining: admin area (~207 files)** — the bulk. Run in `parallel-burndown` sub-waves by feature (schedule-board, customers, equipment/fleet/inventory, invoicing, team, jobs, settings, maintenance, etc.). Then components long-tail + the debug/rare pages last. Honest cost note: full exhaustive sweep is significant Claude spend — prioritize daily-use screens; the app already works with mixed colors (polish, not a blocker).
