...
- [ ] **Duplicate people now show TWICE in the operator dropdown.** Opening the operator slot to supervisors and ops managers (Aug 9) surfaced the roster duplicates from batch 4a: **"David" and "David schadt"** are both active supervisors, and **"Andres Altamirano" and "Andres Altamirano P"** are both active operations managers. The board is keyed on NAME, so a duplicate is not just clutter — dispatching to the wrong one sends the ticket to an account the person doesn't use. Deactivate or merge the stale row of each pair. (Ties to batch 4a.)
