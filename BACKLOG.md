...
- [ ] **P1 — Configurable start time + late-entries page** (Task 3, `/architecture`) — standard start time set in the timecard dashboard = the >7-min-late threshold; per-day overrides (every-other-Mon safety-training → 6:30 AM for everyone); future per-job ticket start-time override (schedule form "different start time"). New "Late entries" management page. **Design doc:** `docs/plans/START_TIME_LATE_PLAN.md` (schema + resolution order). Multi-session; design before building.
- [ ] **P1 — Email UI redesign** (Task 2, deep-research `wfc1x1rbs` DONE) — adopt **react-email** (Resend's own lib; pass component to `resend.emails.send({ react })`). Glossy blue/red/white brand palette, bigger logo (with a solid background plate so dark-mode can't eat it), light + dark readable, bulletproof table layout. Create `emails/` template source + a rendered reference folder. All sends still via `lib/email.ts` (verified domain `admin.pontifexindustries.com`).
- [ ] **P2 — Geofence detail in time-edit-requests + remote clock-in surfacing** (Task 4 remainder) — show the clock-out geofence distance (now in miles/ft) + time delta in the edit-requests view so admins can adjust time when an operator forgot to request; in pending, surface remote clock-ins' submitted **photo + GPS coords** (not just "remote").
