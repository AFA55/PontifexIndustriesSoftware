/**
 * CustomerEnRouteEmail — "Your crew is on the way".
 *
 * Sent to the job's site contact when the operator taps "In Route".
 * White-label: company name + colors from the recipient TENANT'S branding.
 * CTA opens the customer's portal magic-link so they can track job status.
 */
...
export interface CustomerEnRouteEmailProps {
...
export const PreviewProps: CustomerEnRouteEmailProps = {
  branding: {
...
export default function CustomerEnRouteEmail({
  branding,
  customerName,
  jobAddress,
  jobNumber,
  ...
  return (
    <BrandedEmail
      branding={branding}
      preview={`Your crew is on the way — ${companyName}`}
    ...
    >
      <Section style={{ padding: '36px 0 0' }}>
        ...
        <Text style={{ margin: '0 0 24px', color: '#475569', fontSize: '16px', lineHeight: '1.6' }}>
          Good news — your {companyName} crew is on the way to your job site. We&apos;ll get to work as
          soon as we arrive.
        </Text>
      </Section>
      ...
      <Section style={{ padding: '0 0 24px' }}>
        <CTAButton href={portalUrl} label="Track Your Job" color={brandColor} />
      </Section>
