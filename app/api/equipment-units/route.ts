...
export async function GET(request: NextRequest) {
  try {
    ...
    // Compute aggregate stats (tenant-scoped totals for dashboard cards)
    let statsQueryAll = supabaseAdmin.from('equipment_units').select('*', { count: 'exact', head: true });
    let statsQueryActive = supabaseAdmin.from('equipment_units').select('*', { count: 'exact', head: true }).in('lifecycle_status', ['active', 'in_use']);
    let statsQueryService = supabaseAdmin.from('equipment_units').select('*', { count: 'exact', head: true }).in('lifecycle_status', ['needs_service', 'in_maintenance', 'damaged']);
    let statsQueryRetired = supabaseAdmin.from('equipment_units').select('*', { count: 'exact', head: true }).eq('lifecycle_status', 'retired');
    if (tenantId) {
      statsQueryAll = statsQueryAll.eq('tenant_id', tenantId);
      statsQueryActive = statsQueryActive.eq('tenant_id', tenantId);
      statsQueryService = statsQueryService.eq('tenant_id', tenantId);
      statsQueryRetired = statsQueryRetired.eq('tenant_id', tenantId);
    }
    const [
      { count: totalAll },
      { count: activeCount },
      ...
      { count: retiredCount },
    ] = await Promise.all([statsQueryAll, statsQueryActive, statsQueryService, statsQueryRetired]);
