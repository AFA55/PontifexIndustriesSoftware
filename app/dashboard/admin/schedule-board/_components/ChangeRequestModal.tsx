    ...
    <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4">
      <div className="bg-gray-900 rounded-2xl border border-gray-700 w-full max-w-md">
        <div className="flex items-center justify-between p-6 border-b border-gray-800">
          <div>
            <h2 className="text-lg font-semibold text-white">Request Schedule Change</h2>
            <p className="text-gray-400 text-sm mt-0.5">{job.job_number} — {job.customer_name}</p>
          </div>
        ...
        <div className="p-6 space-y-4">
          {/* Type selector */}
          <div>
            <label className="text-sm text-gray-400 mb-2 block">What needs to change?</label>
            <div className="space-y-2">
              {REQUEST_TYPES.map(type => (
                ...
                >
                  <type.icon className={`w-4 h-4 mt-0.5 flex-shrink-0 ${requestType === type.key ? 'text-purple-400' : 'text-gray-400'}`} />
                  <div>
                    <div className={`text-sm font-medium ${requestType === type.key ? 'text-white' : 'text-gray-300'}`}>{type.label}</div>
          ...
          {/* Reason */}
          <div>
            <label className="text-sm text-gray-400 mb-1 block">Reason / Notes *</label>
            <textarea
              value={reason}
