...
export default function CrewScheduleGrid({ onDateClick }: CrewScheduleGridProps) {
  ...
  if (loading && !data) {
    return (
      <div className="flex items-center justify-center py-20">
        ...
        <span className="ml-3 text-gray-600 font-medium">Loading crew schedule...</span>
      </div>
    );
  }
  ...
  return (
    <div ref={gridRef} className="space-y-4">
                  ...
                  >
                    <div className="leading-tight">
                      ...
                      {(d.dayNum === 1 || d === data.dates[0]) && (
                        <div className="text-[9px] text-gray-500 uppercase">{d.month}</div>
                      )}
                    </div>
                  </th>
                ))}
                ...
                return (
                  ...
                  >
                    ...
                    <td className={`sticky left-0 z-10 px-4 py-2 text-sm font-semibold whitespace-nowrap ${
                      isUnassigned
                        ? 'bg-red-50 text-red-700 border-r-2 border-red-200'
                        : rowIdx % 2 === 0
                          ? 'bg-white text-gray-800 border-r border-gray-100'
                          : 'bg-gray-50 text-gray-800 border-r border-gray-100'
                    }`}>
                      <div className="flex items-center gap-2">
                        {isUnassigned ? (
                        ...
                        <div className="truncate max-w-[90px]">
                          ...
                          {!isUnassigned && row.role === 'apprentice' && (
                            <div className="text-[10px] text-gray-400 font-normal">Helper</div>
                          )}
                        </div>
                      </div>
                    </td>
                      ...
                      return (
                        ...
                        >
                          ...
                          {popoverCell?.opId === row.operator_id && popoverCell?.date === cell.date && popoverCell.jobs.length > 0 && (
                            <div className="absolute z-50 top-full left-1/2 -translate-x-1/2 mt-1 w-64 bg-white rounded-2xl shadow-2xl border border-gray-200 p-3 text-left">
                              ...
                              <div className="space-y-1.5 max-h-48 overflow-auto">
                                {popoverCell.jobs.map(job => (
                                  <div key={job.id} className="flex items-start gap-2 p-2 bg-gray-50 rounded-lg">
                                    <Briefcase className="w-3 h-3 text-blue-500 mt-0.5 flex-shrink-0" />
                                    <div className="min-w-0">
                                      <p className="text-xs font-semibold text-gray-800 truncate">{job.job_number}</p>
                                      ...
                                      <div className="flex items-center gap-2 mt-0.5">
                                        ...
                                        <span className={`text-[10px] px-1.5 py-0.5 rounded-full font-medium ${
                                          ...
                                          job.status === 'in_route' ? 'bg-amber-100 text-amber-700' :
      ...
      {/* Mobile Legend */}
      <div className="flex sm:hidden items-center justify-center gap-4 text-xs text-gray-500">
