...
export default function NotesDrawer({ job, notes, onAddNote, onClose }: NotesDrawerProps) {
  ...
  return (
    <>
      ...
      <div className="fixed right-0 top-0 bottom-0 w-full sm:w-[400px] bg-white dark:bg-[#1a0f35] shadow-2xl z-[80] flex flex-col animate-in slide-in-from-right duration-300">
        {/* Header */}
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-3">
          {notes.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-12 text-gray-400 dark:text-white/40">
              <MessageSquare className="w-10 h-10 mb-2 text-gray-300 dark:text-white/20" />
              <p className="font-semibold text-gray-500 dark:text-white/50">No notes yet</p>
            ...
            notes.map((note) => (
              <div key={note.id} className="bg-gray-50 dark:bg-white/[0.05] rounded-xl p-3 border border-gray-200 dark:border-white/10">
                ...
                <p className="text-sm text-gray-700 dark:text-white/70 ml-8">{note.text}</p>
              </div>
            ))
          )}
        </div>
        ...
        {/* Add note input */}
        <div className="border-t border-gray-200 dark:border-white/10 p-4 bg-gray-50 dark:bg-white/[0.03]">
          <div className="flex items-end gap-2">
            <textarea
              ...
              rows={2}
              className="flex-1 px-3 py-2 border-2 border-gray-200 dark:border-white/10 rounded-xl focus:border-green-500 focus:ring-2 focus:ring-green-200 text-sm resize-none bg-white dark:bg-white/[0.05] text-gray-900 dark:text-white placeholder:text-gray-400 dark:placeholder-white/30"
              onKeyDown={(e) => {
