-- Add operator live-progress timestamp columns to schedule_board_view so the
-- schedule board can render a live operator-status pill (En route / On site /
-- Working / Done) derived client-side — no per-card API calls.
--
-- These columns already exist on job_orders; the view simply omitted them.
-- Additive, idempotent (full view replace), no data risk.
...
SELECT
  ...
  -- Live operator-progress timestamps (drive the schedule-board live-status pill)
  jo.in_route_at,
