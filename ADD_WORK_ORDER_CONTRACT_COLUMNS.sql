-- Add columns to job_orders table for Work Order Agreement tracking
...
-- Update the active_job_orders view to include new columns
CREATE OR REPLACE VIEW active_job_orders AS
SELECT
  jo.*,
  o.status AS operator_status,
  jo.work_order_signed,
  jo.work_order_signed_at,
  jo.cut_through_authorized,
  jo.completion_signed_at
FROM job_orders jo
LEFT JOIN operators o ON jo.assigned_operator_id = o.id
